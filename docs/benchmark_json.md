...
- **グローバルキー辞書**: 全オブジェクトキーをヘッダに1回だけ格納し、オブジェクト内は辞書インデックスで参照。辞書エントリはタグなしのコンパクト形式（長さ+データのみ）
- **型付き配列 (Typed Array)**: 整数/浮動小数点のみの配列は要素ごとのタグを省略し、最小幅で格納
- **インライン小整数**: 0-127 の非負整数は 1 バイト（タグ自体に値を埋め込み）で格納。JSON テキストの `"30"` (2バイト) より小さい
- **コンパクト整数エンコーディング**: 128 以上の整数は値の大きさに応じて 1/2/4/8 バイト幅を選択
- **コンパクト真偽値**: true/false を専用タグ 1 バイトで格納（JSON テキストの `true`/`false` (4-5バイト) より大幅に小さい）
- **コンパクトカウント**: オブジェクト (≤255フィールド) と配列 (≤255要素) のカウントを uint8 で格納。大きい場合のみ Large タグで uint16/uint32 を使用
...
|--------|-------------|--------------|------------|------|
| SmallObject | 39 | 37 | **94.9%** | `{"name":"alice","age":30,"active":true}` |
| MediumObject | 147 | 151 | **102.7%** | 7 フィールド、ネストあり |
| LargeObject | 754 | 705 | **93.5%** | 配列・ネストオブジェクト含む複雑な構造 |
| ArrayOf100Ints | 291 | 105 | **36.1%** | 整数 100 要素の配列 |
| DeeplyNested | 132 | 155 | **117.4%** | 10 段ネストのオブジェクト |

**考察:** インライン小整数、コンパクト真偽値、辞書エントリのタグ省略、コンパクトカウント (uint8) の最適化により、SmallObject は **94.9%**、LargeObject は **93.5%** と JSON テキストを下回る。MediumObject (102.7%) もほぼ同等。DeeplyNested (117.4%) はネスト階層ごとのオブジェクトヘッダ＋エントリテーブルのオーバーヘッドが残るが、前回 (125.8%) から大幅改善。整数配列は Typed Array (最小幅パッキング) + コンパクトカウントにより **36.1%** まで削減される。
...
| データ | 旧実装 (ns/op) | 旧 (allocs) | 新実装 (ns/op) | 新 (allocs) | 高速化 |
|--------|---------------|-------------|---------------|-------------|--------|
| SmallObject | 754 | 19 | **108** | 3 | **7.0x faster** |
| MediumObject | 2,344 | 60 | **371** | 3 | **6.3x faster** |
| LargeObject | 11,682 | 245 | **1,990** | 3 | **5.9x faster** |

**考察:** 旧実装は `Decode` (フルデコード → `map[string]any`) + `convertForJSON` + `json.Marshal` の3段階処理で、中間の Go 値アロケーションが支配的だった。新実装はバイナリから直接 JSON 文字列を構築し、辞書キーも `dictEntry` のバイトオフセットから直接出力に書き込むため、中間アロケーションを完全回避。アロケーションは `readDictOffsets` の entries スライス、出力バッファ、最終 `string` 変換の **3 回のみ**。
...
| `SELECT data FROM docs` (100行) | 5,860 | 117 | 34,989 | 517 | JSON が **6.0x faster** |
| `SELECT data FROM docs WHERE id = 50` | 454 | 12 | 746 | 16 | JSON が **1.6x faster** |
...
- **SELECT data (全行)**: JSONB は出力時に `ToJSON` (バイナリ → 直接 JSON 構築) が 100 行分発生するため **6.0 倍**の差。ToJSON の直接構築最適化により、旧実装 (34.2x) から大幅に改善。
- **SELECT data WHERE id = 50 (PK lookup)**: 1 行のみで、ToJSON 最適化により差は **1.6 倍**まで縮小（旧実装では 5.2x）。
...
| 観点 | JSON | JSONB |
|------|------|-------|
| 格納サイズ (一般的なオブジェクト) | baseline | **93.5-117.4%** (同等〜小さい) |
| 格納サイズ (整数配列) | baseline | **36%** (大幅削減) |
...
| ToJSON 変換 (jsonb pkg) | — | **5.9-7.0x faster** (直接構築最適化) |
| SQL INSERT | **3.1x faster** | バイナリ変換コスト |
| SQL SELECT data 全行 (パスなし) | **6.0x faster** | ToJSON 変換コスト |
| SQL SELECT data PK lookup (パスなし) | **1.6x faster** | ToJSON 変換コスト |
| SQL SELECT id (JSON列不使用) | 同等 | 同等 |

JSONB は INSERT 時のエンコードコストと引き換えに、読み取り時の部分アクセスで大きな性能優位性を持つ。空間効率面では、インライン小整数・コンパクト真偽値・辞書タグ省略・コンパクトカウント (uint8) の最適化により、一般的なオブジェクトで JSON テキストと同等〜それ以下のサイズを実現（SmallObject 94.9%, LargeObject 93.5%）。フルデコードでも JSON テキストの Unmarshal より 2.3-2.9 倍高速であり、jsonb パッケージレベルの部分アクセスでは 8-60 倍の高速化を実現する。特に `ExistsPath` は辞書のバイトオフセット参照により文字列デコードを完全に回避し、26-28 倍の高速化を達成。実際の SQL クエリ実行でも JSON_VALUE で 4.5 倍、JSON_EXISTS で 8.1 倍の高速化が確認されており、JSON_QUERY、JSON_VALUE、JSON_EXISTS などの関数でパス指定のアクセスが頻繁に行われるワークロードにおいて、JSONB 型の利点が顕著に現れる。

一方、パスアクセスを伴わないワークロードでは JSON 型が有利。INSERT は 3.1 倍、SELECT data (全行返却) は 6.0 倍 JSON 型の方が高速。ToJSON の直接構築最適化により、旧実装の 34.2 倍から大幅に改善された。PK lookup (1行) では差は 1.6 倍まで縮小しており、JSONB のデメリットは限定的。JSON 列に触れないクエリ (`SELECT id`) では両型の性能は同等。
