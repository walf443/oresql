...
func optimizeExpr(expr ast.Expr) ast.Expr {
	...
	case *ast.BinaryExpr:
		...
		if isConstant(left) && isConstant(right) {
			lv := constValue(left)
			rv := constValue(right)
			result, err := evalComparison(lv, e.Op, rv)
			if err == nil {
	...
	case *ast.ArithmeticExpr:
		...
		if isConstant(left) && isConstant(right) {
			lv := constValue(left)
			rv := constValue(right)
			result, err := evalArithmetic(lv, e.Op, rv)
			if err == nil {
	...
	case *ast.InExpr:
		if e.Subquery != nil {
			return expr // don't optimize subquery IN
		}
		left := optimizeExpr(e.Left)
		if !isConstant(left) {
			return expr // left is not constant, skip
		}
		allConst := true
	...
	case *ast.BetweenExpr:
		...
		if isConstant(left) && isConstant(low) && isConstant(high) {
			lv := constValue(left)
			lowV := constValue(low)
			highV := constValue(high)
			if lv == nil {
				return &ast.BoolLitExpr{Value: false}
			}
			geq, err1 := evalComparison(lv, ">=", lowV)
			leq, err2 := evalComparison(lv, "<=", highV)
